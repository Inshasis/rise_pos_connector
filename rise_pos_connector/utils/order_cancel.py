            ...
            if shop.sync_orders:
                ...
                if response.status_code == 200:
                    ...
                    for ord_feach in ord['result']['orders']:
                        ...
                        elif ord_feach['payment_type'] == "PAYT0000":
                            # Transaction Id Check In Sales Invoice
                            sales_inv = frappe.get_list('Sales Invoice', fields=['custom_order_id'])
                            check_inv = {'custom_order_id': ord_feach['order_id']}
                            
                            if check_inv in sales_inv:
                                pay_reff = frappe.db.sql("select parent from `tabPayment Entry Reference` where reference_doctype = 'Sales Invoice' AND reference_name = %s", ord_feach['order_id'])
                                for pay_entry in pay_reff:
                                    ...
                                    # Sales Invoice Cancel
                                    si = frappe.db.get_value("Sales Invoice", {'custom_order_id': ord_feach['order_id']}, "name")
                                    si_doc = frappe.get_doc("Sales Invoice", si)
                                    si_doc.cancel()
                                    frappe.db.commit()
